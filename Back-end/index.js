...
app.put('/activities/:id', (req, res) => {
    ...
    fs.readFile(activitiesFile, 'utf8', (err, data) => {
        ...
        fs.writeFile(activitiesFile, JSON.stringify(arr, null, 2), (err2) => {
            ...
            res.json({ success: true, activity: arr[idx] });
        });
    });
});
...
app.delete('/activities/:id', (req, res) => {
    ...
    fs.readFile(activitiesFile, 'utf8', (err, data) => {
        ...
        // ลบไฟล์ภาพทั้งหมดถ้ามี
        if (activity.imagePath && Array.isArray(activity.imagePath)) {
            activity.imagePath.forEach(imgPathStr => {
                const imgPath = path.join(__dirname, '../Back-end', imgPathStr.replace(/^\//, ''));
...
// ----------------- ผู้มาเยี่ยมชม (Visitor) -----------------
const visitorStorage = multer.diskStorage({
    destination: (req, file, cb) => {
        cb(null, path.join(__dirname, '../Back-end/uploads/visitors'));
    },
    filename: (req, file, cb) => {
        cb(null, Date.now() + path.extname(file.originalname));
    }
        ...
        if (idx !== -1) {
            ...
            if (visitors[idx].images && visitors[idx].images.length > 0) {
                visitors[idx].images.forEach(img => {
                    const imgPath = path.join(__dirname, '../Back-end/uploads/visitors', img);
...
app.post('/news', (req, res) => {
    ...
    console.log('Received POST /news:', req.body);
